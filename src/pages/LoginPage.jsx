...
const LoginPage = ({ onLogin, autoLoggingIn, notice }) => {
    ...
    // A passed-in notice (failed auto-login / idle logout) shows until the
    // user starts a fresh attempt; a manual-submit error takes precedence.
    const banner = error
            ...
            <div className="w-full max-w-sm">
                ...
                <div className="bg-github-bg-secondary border border-github-border rounded-lg p-6">
                    <div className="space-y-4">
                        <div>
                            <label htmlFor="gh-username" className="block text-sm font-medium text-github-text mb-1.5">
                                GitHub Username
                            </label>
                            <input
                                id="gh-username"
                        ...
                        <div>
                            <label htmlFor="gh-token" className="block text-sm font-medium text-github-text mb-1.5">
                        ...
                        {banner && (
                            <div
                                ...
                                className={`flex items-start gap-2 text-xs p-3 rounded-md border ${banner.tone === 'error'
                                    ? 'text-github-status-closed bg-github-status-closed/10 border-github-status-closed/20'
                                    : 'text-github-text-secondary bg-github-bg-tertiary border-github-border'
                                    }`}
                            >
                                {banner.tone === 'error' ? (
                ...
                <p className="mt-4 text-[11px] text-github-text-secondary text-center leading-relaxed">
                    A token you type here is used only in your browser to query the GitHub
                    GraphQL API, kept in React state, and discarded when you sign out or
                    close the tab. It is never persisted and never sent to any server run
                    by this site.
                </p>
            </div>
        </div>
